 ...
 *
 * Usage:
 *   - Automatically triggered when session closes
 *   - Manual trigger: claude-code /summarize
 */
...
const PROMPT_FILE = path.join(process.cwd(), '.claude', 'hooks', 'summarize-prompt.txt');
...
function getGitChanges() {
  try {
    ...
    return status
      ...
      .map((line) => {
        const [status, file] = line.slice(3).split(/\s+/);
        const icons = {
          M: '📝',
          A: '✨',
          D: '🗑️',
 ...
 * @returns {Array} - List of referenced files
 */
...
module.exports = { createSummary, generateSessionFileName };
